...
# (i)处理取值少于5的变量：
#   如果每种类别同时包含好坏样本，无需分箱；
#   如果有类别只包含好坏样本的一种，需要合并(两端都要0%， 100%)

merge_bin_dict = {}  # 存放需要合并的变量以及其合并方法 {'feature': {'value0': 'Bin_0', 'value1': 'Bin_1', ...}}
...
# （ii）处理取值大于5的变量：用bad rate进行编码，放入连续型变量里进行处理
...
# （iii）对连续型变量进行分箱，包括（ii）中新添加的变量
continous_merged_dict = {}
for col in num_features:
    print("{} is in processing".format(col))

    special_attr = [] if -1 not in set(trainData[col]) else [-1]
    max_interval = 5 # 不包含特殊值的情况，有没有特殊值，都是 max_interval，最小为2

    while True:
        cutOff = ChiMerge(trainData, col, 'y', max_interval=max_interval, special_attribute=special_attr, minBinPcnt=0)
        trainData[col+'_Bin'] = trainData[col].map(lambda x: AssignBin(x, cutOff, special_attribute=special_attr))
        monotone = BadRateMonotone(trainData, col+'_Bin', 'y', special_attribute=special_attr)   # 检验分箱后的单调性是否满足
    ...
    newVar = col + '_Bin'
    trainData[newVar] = trainData[col].map(lambda x: AssignBin(x, cutOff, special_attribute=special_attr))
    var_bin_list.append(newVar)

    # Original Code
    # if -1 not in set(trainData[col]):   # 连续变量的情况：－1会当成特殊值处理。如果没有－1，则所有取值都参与分箱
    ...
    #     while (not monotone):
    #         max_interval -= 1
    #         # 如果有－1，－1的bad rate不参与单调性检验， 所以出去 -1 的组，最小分为两箱的时候，必然单调
    #         cutOff = ChiMerge(trainData, col, 'y', max_interval=max_interval, special_attribute=[-1], minBinPcnt=0)
    #         trainData[col + '_Bin'] = trainData[col].map(lambda x: AssignBin(x, cutOff, special_attribute=[-1]))
    #         if max_interval == 2:
    #             # 当分箱数为2时，必然单调
