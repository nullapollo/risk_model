...
from crsc.univ.univariate import *
...
allData1 = allData[allData.term == 36]

# 一开始就切分好数据集，之后对测试集，应用各类分箱等操作处理
trainData0, testData0 = train_test_split(allData1, test_size=0.4, random_state=123)
trainData = trainData0.copy()
...
trainData['desc_clean'] = trainData['desc'].apply(lambda x: DescExisting(x))

# 处理日期 earliest_cr_line 的格式不统一，需要统一格式且转换成日期字符串
trainData['app_date_clean'] = trainData['issue_d'].apply(lambda x: ConvertDateStr2(x, '%b-%y'))
trainData['earliest_cr_line_clean'] = trainData['earliest_cr_line'].apply(lambda x: ConvertDateStr2(x, '%b-%y'))

# 处理mths_since_last_delinq。注意原始值中有0，所以用－1代替缺失
trainData['mths_since_last_delinq_clean'] = trainData['mths_since_last_delinq'].apply(lambda x: MakeupMissing(x))

trainData['mths_since_last_record_clean'] = trainData['mths_since_last_record'].apply(lambda x: MakeupMissing(x))

trainData['pub_rec_bankruptcies_clean'] = trainData['pub_rec_bankruptcies'].apply(lambda x: MakeupMissing(x))
...
con_analysis = con_vars_dist(trainData, num_features)
